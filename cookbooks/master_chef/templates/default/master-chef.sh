...
(
  LOCAL_STORAGE_FILE=$LOCAL_STORAGE_FILE REPOS_STATUS_FILE=$REPOS_STATUS_FILE MASTER_CHEF_CONFIG=$MASTER_CHEF_CONFIG sudo -E /opt/chef/bin/chef-solo -c /opt/chef/etc/solo.rb.erb
  if [ "$?" = 0 ]; then
