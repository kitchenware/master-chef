...
execute "install jboss with official zip" do
  ...
  command "cd #{node.jboss.home} && curl --location #{node.jboss.zip_file} -o && unzip jboss-as-#{node.jboss.version}.zip && jboss-as-#{node.jboss.version}.zip && rm jboss-as-#{node.jboss.version}.zip"
