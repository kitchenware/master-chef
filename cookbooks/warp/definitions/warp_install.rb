...
define :warp_install, {
  ...
  install_env.merge!({"git_protocol" => "http"}) if node.git.auto_use_http_for_github && (ENV['BACKUP_http_proxy'] || node[:no_external_ssh] || node.git[:force_use_http_for_github])
